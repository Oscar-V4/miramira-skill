...
**MiraMira 是一个镜子般的 Agent Skill：一次只问一个问题，持续 grill 你的计划。**  
它简单到不可思议：给它一个计划，它会把下一个最重要的问题映照回来，并同时给出推荐回答。

- 把计划、设计、产品方向和技术选择压缩成更锋利的问题。
- 沿着决策树一条分支一条分支地推进。
- 一次只问一个问题。
- 如果答案能通过本地代码库探索得到，就先探索代码库。
- 将 Matt Pocock 的 `grill me` 流程重新诠释成 MiraMira 的镜子概念。
