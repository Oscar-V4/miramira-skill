...
**MiraMira는 계획을 거울처럼 비추고, 한 번에 한 질문씩 집요하게 grill하는 에이전트 스킬입니다.**  
믿을 수 없을 만큼 단순합니다. 계획을 말하면, MiraMira는 가장 중요한 다음 질문 하나를 던지고, 그 질문에 대한 추천 답까지 함께 제시합니다.

- 계획, 설계, 제품 방향, 기술 선택을 질문으로 압축합니다.
- 결정 트리의 가지를 하나씩 따라가며 모호함을 줄입니다.
- 질문은 한 번에 하나만 던집니다.
- 답이 코드베이스 탐색으로 해결되면 먼저 탐색합니다.
- Matt Pocock의 `grill me` 흐름을 MiraMira의 거울 컨셉으로 재해석했습니다.
...
| 상황 | 이렇게 말하면 됩니다 |
|---|---|
| 아이디어가 그럴듯한지 보고 싶을 때 | `$miramira 이 제품 아이디어를 grill해줘. 한 번에 한 질문씩.` |
| 설계가 애매하게 느껴질 때 | `$miramira 이 아키텍처 계획의 가장 위험한 모호함부터 물어봐줘.` |
| 결정을 미루고 있을 때 | `$miramira 이 선택지를 비교하면서 다음 질문과 추천 답을 줘.` |
| PRD나 기능 범위를 다듬을 때 | `$miramira 이 기능 계획을 사용자 가치 기준으로 grill해줘.` |
| 구현 전에 빠르게 흔들어보고 싶을 때 | `$miramira 이 구현 계획에서 먼저 확인해야 할 질문 하나만 던져줘.` |
